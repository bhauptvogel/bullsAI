...
def simulate():
    ...
    for i in range(int(left/2)):
        ...
        if player1 != 'PLAYER' and player2 != 'PLAYER':
            ...
            while player1_game_wins < GAMES_TO_WIN and player2_game_wins < GAMES_TO_WIN:
                ...
                LEGS_TO_WIN = 5
                player_beginning = (player1_game_wins + player2_game_wins + 1) % 2 + 1
                ...
                game_winner = 1 if player1_leg_wins > player2_leg_wins else 2
                ...
                if game_winner == 1:
                    ...
                    raise ValueError("Game winner is not valid")
                
                print(f"Game {player1_game_wins + player2_game_wins}: {player1} {player1_game_wins} - {player2_game_wins} {player2}")


            winner = player1 if player1_game_wins > player2_game_wins else player2
            loser = player1 if player1_game_wins < player2_game_wins else player2


            results = pd.concat([results, pd.DataFrame([[player1, player2, f'{player1_game_wins} - {player2_game_wins}']], columns=['Player 1', 'Player 2', 'Result'])])
...
def play(game_number: int = 1):
    ...
    for i in range(int(left/2)):
        ...
        if player1 == 'PLAYER' or player2 == 'PLAYER':
            ...
            first_to_throw = 'player' if game_number % 2 == 1 else 'bot'
            result = handle_game(game_id=game_id, bot_average=opponent_average, sets_to_win=1, legs_to_win=5, first_to_throw=first_to_throw, starting_points=501, bot_name=opponent_name, save_log_location='fantasy/cup/game_logs/')
...
def main():
    ...
    parser.add_argument("--game_number", type=int, default=1, help="The game number to play")
